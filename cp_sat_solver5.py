"""
Stage-1 only CP-SAT LCV optimizer (partial loads allowed, min-load, slack)
Objective: Minimize (BIG_SLACK * total_slack + total_trips)
Cost is calculated for reporting but NOT used in the objective.
TUNED FOR: ~160k kg dispatch with ~7.5-8 lakh cost
"""
...
# Solver config (tune these)
TIME_LIMIT_SEC = 900      # Increased from 240 for better solutions
NUM_WORKERS = 8
MAX_NEAR_DISTANCE_KM = 25  # Relaxed from 20 to allow more trip options
# Business tuning
MIN_LOAD_KG = 120         # Reduced from 130 to allow more efficient packing
BIG_SLACK = 2_000_000     # Increased from 1M to prioritize demand fulfillment more
DELIVERY_WEIGHT = 2000    # Increased to reward higher 
EXTRA_ROTATION_LCVS = {965, 967, 5833, 3682, 3727} 
...
MAX_COST_PAISE = 81000000  # Relaxed from 79M to 81M (810k rupees)
TARGET_DISPATCH_KG = 160000  # Target dispatch
...
def build_stage1_model(stations_df, lcv_list):
    """
    Build CP-SAT model for Stage-1 only.
    Objective: Minimize BIG_SLACK * total_slack - DELIVERY_WEIGHT * delivered + total_trips
    Returns: model, x_vars, y_vars, slack_vars, capacities_map, rate_paise_map
    """
    ...
    # Create x (int kg) and y (bool) variables
    x = {}
    y = {}
    for lcv in lcv_list:
        l = int(lcv['lcv_no'])
        ...
        for sid, srow in stations_df.iterrows():
            for o in ORIGINS:
                ...
                x[(l, sid, o)] = vx
                y[(l, sid, o)] = vy
                # linking if y=0 => x=0; if y=1 => x <= cap
                model.Add(x[(l, sid, o)] <= cap * y[(l, sid, o)])
                # minimum load if enabled
                if MIN_LOAD_KG > 0:
                    model.Add(x[(l, sid, o)] >= MIN_LOAD_KG * y[(l, sid, o)])

    # Restrict extra trips to nearby stations
    ...
    # Slack vars per station (kg, integer) - unmet demand
    slack = {}
    for sid, srow in stations_df.iterrows():
        max_d = int(srow.get('Capacity', 0))
        slack[(sid,)] = model.NewIntVar(0, max_d, f"slack_s{sid}")

    # Station demand constraints: sum(x) + slack >= demand
    for sid, srow in stations_df.iterrows():
...
def run_stage1_only(stations_df, lcv_list):
    ...
    total_dispatched = 0
    total_slack = 0
    # Precompute rate in paise for reporting
    rate_paise = {}
    for lcv in lcv_list:
        l = int(lcv['lcv_no'])
        for sid, srow in stations_df.iterrows():
